...
export default function App() {
  ...
  const fetchData = async (currentFilter) => {
    ...
    setError(null);
    try {
      const url = `${API_BASE}?action=report&filter=${currentFilter}`;
        ...
        <div className="max-w-md mx-auto flex items-center justify-between">
          <div>
            ...
            <p className="text-[10px] text-blue-100 flex items-center mt-0.5">
              ...
              {data?.updateTime ? `Cập nhật: ${data.updateTime}` : 'Đang tải...'}
            </p>
          </div>
          <button 
            ...
            <div className="grid grid-cols-4 gap-1.5 mb-3">
              <div className="col-span-4 bg-white rounded-xl shadow-xs border border-slate-100 p-3 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xl font-black text-slate-800 mt-0.5">{data.summary.totalProduct}</p>
                </div>
              ...
              <div className="bg-white rounded-xl shadow-xs border border-slate-100 p-2 text-center">
                ...
                <p className="text-sm font-black text-green-600 mt-0.5">{data.summary.onSchedule}</p>
              </div>

              <div className="bg-white rounded-xl shadow-xs border border-slate-100 p-2 text-center">
                <p className="text-[8px] font-bold text-slate-400 uppercase">TRỄ</p>
                <p className="text-sm font-black text-red-600 mt-0.5">{data.summary.delay}</p>
              </div>

              <div className="bg-white rounded-xl shadow-xs border border-slate-100 p-2 text-center">
                <p className="text-[8px] font-bold text-slate-400 uppercase">ĐANG SX</p>
                <p className="text-sm font-black text-blue-600 mt-0.5">{data.summary.producing}</p>
              </div>

              <div className="bg-white rounded-xl shadow-xs border border-slate-100 p-2 text-center">
                <p className="text-[8px] font-bold text-slate-400 uppercase">VƯỢT</p>
                <p className="text-sm font-black text-purple-600 mt-0.5">{data.summary.ahead}</p>
              </div>
            </div>
